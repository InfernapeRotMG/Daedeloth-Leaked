...
namespace wServer.realm.entities.player
{
    partial class Player
    {
        ...
        public void RequestTrade(RealmTime time, RequestTradePacket pkt)
        {
            ...
            {
                SendError("Head Spriters can't trade anything!");
                return;
            }
            if (Client.Account.Rank >= 11)
            {
                SendError("Developers can't trade anything!");
                return;
            }
            if (Client.Account.Rank >= 12)
            {
                SendError("Head Developers can't trade anything!");
                return;
            }
            if (Client.Account.Rank >= 13)
            {
                SendError("Special Players can't trade anything!");
                return;
            }
